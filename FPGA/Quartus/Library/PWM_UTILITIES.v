...
(
	...
	reg [NUM_BITS-1:0] current_output_value;
	...
	always@(posedge clk) current_output_value <= output_value;
...
module Step_Controller#(
	...
	assign delta = desired_value > read_value; //desired_value - read_value;
	assign output_value = output_value_reg;
	assign compare = (desired_value == read_value);

	always@(posedge clk) begin
		clk_counter <= (clk_counter>STEP_TIME) ? 0 : clk_counter + 1;
		if (clk_counter == STEP_TIME) output_value_reg <= (delta) ? current_output_value + 1 : current_output_value - 1;
		else output_value_reg <= output_value_reg;
	end
endmodule
...
module Servo_Controller#(
	...
	parameter PWM_COUNT_PERIOD = 1000,
	parameter DUTY_MAX = 255,
	...
	input read_pwm,
	output output_pwm,
	output [NUM_BITS-1:0] read_value
);
//	wire [NUM_BITS-1:0] read_value, controlled_value;
	wire [NUM_BITS-1:0] controlled_value;
	reg [NUM_BITS-1:0] output_value_reg;
	...
	Step_Controller #(
		...
		.clk(clk),
		.current_output_value(output_value_reg),
